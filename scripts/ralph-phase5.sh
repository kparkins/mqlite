#!/usr/bin/env bash
# ralph-phase5.sh - drive Phase 5 implementation story-by-story via ralph,
...
# .omc/phase-05-prd.json, invokes ralph on just that story, waits for it to
# exit, then loops. Stops when every story passes.
#
# Usage:
#   scripts/ralph-phase5.sh                 # run to completion
#   scripts/ralph-phase5.sh --dry-run       # print next story, do nothing
#   scripts/ralph-phase5.sh --max 3         # stop after 3 iterations
#   PRD=/path/to/other-prd.json scripts/ralph-phase5.sh
#
# Env overrides:
#   PRD        path to the prd.json           (default: .omc/phase-05-prd.json)
...
DEFAULT_PLAN_PRD=".omx/plans/prd-phase-05-same-collection-"
DEFAULT_PLAN_PRD="${DEFAULT_PLAN_PRD}multi-writer-crud.md"
DEFAULT_TEST_SPEC=".omx/plans/test-spec-phase-05-same-collection-"
DEFAULT_TEST_SPEC="${DEFAULT_TEST_SPEC}multi-writer-crud.md"
DEFAULT_SOURCE_DOC="docs/STORAGE-UPGRADE-PHASE-05-MULTI-WRITER-CRUD.md"

PRD="${PRD:-.omc/phase-05-prd.json}"
...
PHASE_NAME="$(jq -r '.meta.phase // "Storage Upgrade Phase 5"' "$PRD")"
SOURCE_DOC="$(
  jq -r --arg fallback "$DEFAULT_SOURCE_DOC" \
    '.meta.source_doc // $fallback' "$PRD"
)"
[ -f "$SOURCE_DOC" ] || {
  echo "error: Phase 5 spec not found at $SOURCE_DOC" >&2
  exit 2
}
if [ "$DRY_RUN" = 0 ]; then
...
run_driver() {
  ...
  while kill -0 "$DRIVER_PID" 2>/dev/null; do
    if [ "$(story_passes "$id")" = "true" ]; then
      echo "[ralph-phase5] $id is passes:true; waiting for driver exit."

      waited=0
      while kill -0 "$DRIVER_PID" 2>/dev/null; do
        if [ "$waited" -ge "$EXIT_GRACE_SECONDS" ]; then
          echo "[ralph-phase5] driver still running after passes:true;"
          echo "[ralph-phase5] terminating lingering session for $id."
          stop_driver
      ...
      if [ "$rc" -ne 0 ] && [ "$(story_passes "$id")" = "true" ]; then
        echo "[ralph-phase5] driver exited rc=$rc after $id passed;"
        echo "[ralph-phase5] continuing to the next story."
        return 0
      fi
      return "$rc"
    fi
...
while :; do
  row="$(next_story)"
  if [ -z "$row" ]; then
    echo "[ralph-phase5] all stories pass - done."
    exit 0
  fi

  iter=$((iter + 1))
  if [ "$MAX_ITER" -gt 0 ] && [ "$iter" -gt "$MAX_ITER" ]; then
    echo "[ralph-phase5] stopped after $MAX_ITER iterations (--max)."
    echo "[ralph-phase5] Open: $(open_count)"
    exit 0
  fi
...
blocker finding as a failed story."
  ...
  echo "[ralph-phase5] iter=$iter story=$id priority=$pri"
  echo "[ralph-phase5]   phase: $phase"
  echo "[ralph-phase5]   title: $title"
  echo "[ralph-phase5]   spec:  $spec"
  echo "[ralph-phase5]   plan:  $PLAN_PRD"
  echo "[ralph-phase5]   tests: $TEST_SPEC"
  echo "[ralph-phase5]   driver: $CODEX_BIN"
  echo "[ralph-phase5]   effort: $CODEX_EFFORT"
  echo "[ralph-phase5]   critic:   $CRITIC"
  echo "[ralph-phase5]   log:   $log"

  if [ "$DRY_RUN" = 1 ]; then
    echo "[ralph-phase5] (dry-run) would invoke ralph on $id"
    exit 0
  fi
...
Treat ${PRD} as the canonical Phase 5 execution and test contract.
Use ${spec} only as same-collection multi-writer CRUD source context.
...
Test-cycle policy (speed without quality loss):
  ...
  2. Final gate, run exactly once after the named tests are green and
     immediately before flipping ${id}.passes to true: the canonical
     'cargo test --release --tests' across the full baseline plus any new
     tests required by ${id}. Critic review still uses this canonical run
...
When ${id}.passes is true and verified, run \$cancel and exit."
  ...
  run_driver || {
      rc=$?
      echo "[ralph-phase5] codex exited rc=$rc on $id - see $log" >&2
      exit "$rc"
    }

  still_open="$(story_passes "$id")"
  if [ "$still_open" != "true" ]; then
    echo "[ralph-phase5] $id still has passes:false after run - halting." >&2
    echo "[ralph-phase5] inspect $log and fix before re-running." >&2
    exit 3
  fi

  echo "[ralph-phase5] $id marked passes:true. Remaining open: $(open_count)"
done
