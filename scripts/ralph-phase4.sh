#!/usr/bin/env bash
# ralph-phase4.sh - drive Phase 4 implementation story-by-story via ralph,
...
# .omc/phase-04-prd.json, invokes ralph on just that story, waits for it to
# exit, then loops. Stops when every story passes.
#
# Usage:
#   scripts/ralph-phase4.sh                 # run to completion
#   scripts/ralph-phase4.sh --dry-run       # print next story, do nothing
#   scripts/ralph-phase4.sh --max 3         # stop after 3 iterations
#   PRD=/path/to/other-prd.json scripts/ralph-phase4.sh
#
# Env overrides:
#   PRD        path to the prd.json           (default: .omc/phase-04-prd.json)
...
PHASE_NAME="$(jq -r '.meta.phase // "Storage Upgrade Phase 4"' "$PRD")"
SOURCE_DOC="$(jq -r '.meta.source_doc // "docs/STORAGE-UPGRADE-PHASE-04-RECONCILIATION-AND-CHECKPOINT.md"' "$PRD")"
[ -f "$SOURCE_DOC" ] || { echo "error: Phase 4 spec not found at $SOURCE_DOC" >&2; exit 2; }
if [ "$DRY_RUN" = 0 ]; then
...
while :; do
  row="$(next_story)"
  if [ -z "$row" ]; then
    echo "[ralph-phase4] all stories pass - done."
    exit 0
  fi

  iter=$((iter + 1))
  if [ "$MAX_ITER" -gt 0 ] && [ "$iter" -gt "$MAX_ITER" ]; then
    echo "[ralph-phase4] stopped after $MAX_ITER iterations (--max). Open: $(open_count)"
    exit 0
  fi
  ...
  echo "[ralph-phase4] iter=$iter story=$id priority=$pri"
  echo "[ralph-phase4]   phase: $phase"
  echo "[ralph-phase4]   title: $title"
  echo "[ralph-phase4]   spec:  $spec"
  echo "[ralph-phase4]   executor: $CODEX_BIN"
  echo "[ralph-phase4]   critic:   $CRITIC"
  echo "[ralph-phase4]   log:   $log"

  if [ "$DRY_RUN" = 1 ]; then
    echo "[ralph-phase4] (dry-run) would invoke ralph on $id"
    exit 0
  fi

  prompt="\$ralph --critic=${CRITIC} Implement exactly one story - ${id} - from ${PRD}. \
Use ${spec} as the authoritative Phase 4 storage-upgrade spec and keep the PRD acceptance criteria as the completion checklist. \
...
When ${id}.passes is true and verified, run \$cancel and exit."
  ...
  "$CODEX_BIN" exec \
    ...
    >"$log" 2>&1 || {
      rc=$?
      echo "[ralph-phase4] codex exited rc=$rc on $id - see $log" >&2
      exit "$rc"
    }
  ...
  if [ "$still_open" != "true" ]; then
    echo "[ralph-phase4] $id still has passes:false after run - halting." >&2
    echo "[ralph-phase4] inspect $log and fix before re-running." >&2
    exit 3
  fi

  echo "[ralph-phase4] $id marked passes:true. Remaining open: $(open_count)"
done
