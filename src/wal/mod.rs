...
mod crash_recovery_tests {
    ...
    fn setup_epoch1(db_path: &Path, seed: u32) -> Result<()> {
        ...
        wal.commit(EPOCH1_END - 1, WalPageSize::Small4k, &page_data, EPOCH1_END - 1)?;
        drop(wal);
        drop(main_file);
        Ok(())
    }
    ...
    unsafe fn child_run_scenario(db_path: &Path, scenario: Scenario, seed: u32, write_fd: libc::c_int) {
        ...
        match scenario {
            ...
            Scenario::InsertAtFinalFrame => {
                ...
                let _ = wal.commit(EPOCH2_START + 5, WalPageSize::Small4k, &page_data, EPOCH2_START + 5);
                step!();
                std::thread::sleep(std::time::Duration::from_secs(60));
            }
            Scenario::CheckpointAt25Pct | Scenario::CheckpointAt50Pct | Scenario::CheckpointAt75Pct => {
                ...
                let _ = wal.commit(EPOCH2_START + e2_span - 1, WalPageSize::Small4k, &epoch2_data, EPOCH2_START + e2_span - 1);
    ...
    fn validate(wal: &WalManager, main_file: &mut std::fs::File, scenario: Scenario, seed: u32) -> Result<()> {
        ...
        if matches!(scenario, Scenario::CheckpointAt25Pct | Scenario::CheckpointAt50Pct | Scenario::CheckpointAt75Pct) {
            ...
            for page_no in 1..=CHECKPOINT_PAGES {
                ...
                if page[0] == CHECKPOINT_GARBAGE_FILL {
                    return Err(Error::Internal(format!(
                        "condition (d) FAIL: checkpoint garbage fill {:#04x} found at page {} after WAL recovery [scenario {:?} seed {}]",
        ...
        if matches!(scenario, Scenario::InsertAtFrame10) {
            for i in 0u32..10 {
                ...
                if wal.shm().lookup(page_no).is_some() {
                    return Err(Error::Internal(format!(
                        "condition (d) FAIL: uncommitted page {} in WAL SHM after recovery [InsertAtFrame10 seed {}]",
                        page_no, seed
                    )));
                }
            }
        ...
        if matches!(scenario, Scenario::InsertAtFrame100) {
            for page_no in EPOCH2_START..EPOCH2_END {
                if wal.shm().lookup(page_no).is_some() {
                    return Err(Error::Internal(format!(
                        "condition (d) FAIL: uncommitted page {} in WAL SHM after recovery [InsertAtFrame100 seed {}]",
                        page_no, seed
                    )));
                }
            }
        ...
        if matches!(scenario, Scenario::IndexBuildAtStart | Scenario::IndexBuildMidway | Scenario::IndexBuildAtEnd) {
            for page_no in INDEX_START..INDEX_END {
                if wal.shm().lookup(page_no).is_some() {
                    return Err(Error::Internal(format!(
                        "condition (e) FAIL: uncommitted index page {} in WAL SHM after recovery [scenario {:?} seed {}]",
    ...
    fn run_cycle(scenario: Scenario, seed: u32) -> Result<()> {
        ...
        let header = FileHeader::new(1_700_000_000_000, SALT1, SALT2);
        let wal = WalManager::open_or_create(&db_path, &header, &mut main_file)
            .map_err(|e| Error::Internal(format!(
                "condition (a)+(b) FAIL: WalManager::open_or_create failed after crash [scenario {:?} seed {seed}]: {e}",
                scenario
            )))?;
