//! Parsed representation of a 32 KB leaf B+ tree node and its cells.

use crate::error::{Error, Result};
use crate::storage::page::{
    leaf_page_checksum, LeafPageHeader, LEAF_FLAG_HAS_OVERFLOW, LEAF_HEADER_SIZE, PAGE_SIZE_LEAF,
    PAGE_TYPE_LEAF, VALUE_TYPE_INLINE, VALUE_TYPE_OVERFLOW,
};

use super::layout::leaf_cell_encoded_size;
...
impl LeafCell {
    ...
    ///
    /// Layout: `key_len(2) | key | value_type(1) | value_data`. Routes through
    /// the canonical [`leaf_cell_encoded_size`] so the fit arithmetic matches
    /// the SMO classifier byte-for-byte.
    pub(super) fn encoded_size(&self) -> usize {
        ...
        leaf_cell_encoded_size(self.key.len(), value_size)
    }
}
...
impl LeafNode {
    ...
    pub(super) fn parse(data: &[u8]) -> Result<Self> {
        ...
        crate::storage::close_quadratic_probe::record_leaf_cells_parsed(n as u64);
        ...
        // Cells are written in sorted-key order (the cell-pointer array indexes
        // them ascending), and `binary_search` over `cells` relies on it. This
        // debug-only assertion catches a writer that ever emits an out-of-order
        // page; it compiles out of release builds entirely.
        debug_assert!(
            cells.windows(2).all(|w| w[0].key <= w[1].key),
            "leaf cells must be parsed in ascending key order"
        );
    ...
    pub(super) fn needs_rebalance(&self) -> bool {
        self.cells.len() < super::layout::MIN_LEAF_CELLS
            || self.used_bytes() < super::layout::MIN_LEAF_BYTES
    }
