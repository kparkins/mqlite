//! US-005 PublishSequencer regression tests (§10.19 / §10.19.0 / §10.19.3).
//!
...
//! us005_tests` reached from `publish_sequencer.rs` via `#[cfg(test)]
//! #[path = "us005_tests.rs"] mod us005_tests;`.
...
/// AC #14 / §10.13.9 — for 3 writers at `publish_seq = 1, 2, 3`, every
/// loom interleaving advances `last_published` monotonically and runs the
/// publish closures in dense seq order. A non-adjacent predecessor would
/// either skip a closure or call them out of order, both of which fail
