...
**mqlite Phase 1 has no authentication, no authorization, and no TLS.**
...
This is intentional for Phase 1, which targets local-only use cases (test
doubles, embedded tools, local development). Authentication is planned for
Phase 2.
...
all network interfaces. Phase 1 has no authentication. Use 127.0.0.1 for
local-only access.
```

### No TLS

Phase 1 transmits all data — including document contents — in plaintext.
...
TLS support is planned for Phase 2.
...
This is the primary intended use case for Phase 1.

### Production (Not Recommended in Phase 1)

If you must expose mqlite in production before Phase 2 authentication ships:
...
| Credential theft | N/A — no credentials exist in Phase 1 |
