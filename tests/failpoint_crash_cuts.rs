...
//! - class C after ChainCommit at the retained pre-legacy failpoint;
//! - class D after the retained legacy-named pre-publish failpoint;
...
#[test]
fn test_class_d_chain_committed_publish_absent_recovers_committed() {
    if run_child_if_requested() {
        return;
    }
    run_parent_case(CrashCase {
        test_name: "test_class_d_chain_committed_publish_absent_recovers_committed",
...
fn run_parent_case(case: CrashCase) {
    ...
    assert_legacy_residue_unchanged(baseline, residue);
...
fn assert_legacy_residue_unchanged(baseline: JournalResidue, residue: JournalResidue) {
    assert_eq!(
        residue.legacy_commits, baseline.legacy_commits,
        "Phase 6 crash cuts must not add retired legacy commit frames"
    );
}
