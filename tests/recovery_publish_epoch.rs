...
fn prepare_replay_fixture() -> (tempfile::TempDir, PathBuf) {
    ...
    let target_legacy_frames = crash_harness::scan_legacy_commit_frames(&path)
        .expect("scan legacy commit frames")
        .into_iter()
        .filter(|(start, _end)| *start > last_chain_offset)
        .collect::<Vec<_>>();
    assert!(
        target_legacy_frames.is_empty(),
