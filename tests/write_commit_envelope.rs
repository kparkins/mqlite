...
//!   3. No legacy page-0 commit frame; Phase 6 deleted that ordinary CRUD
//!      authority.
//!
...
/// Per §3.7/§6.2 plus Phase 6 deletion: one successful CRUD commit grows the
/// journal with `...[Logical][ChainCommit]` and no ordinary legacy commit frame.
#[test]
fn commit_envelope_emits_logical_between_allocate_and_chain_commit() {
    ...
    assert_eq!(
        legacy_commit_count, 0,
        "Phase 6 ordinary CRUD must not emit legacy commit frames; kinds={kinds:?}"
    );
}
