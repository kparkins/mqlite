...
//! Phase 1 US-014 / §10.8 #1-14 — `Arc<PublishedCatalog>` identity
//! witness tests.
//!
//! Reuse tests (1-7) drive mutations that MUST reuse the prior
//! `Arc<PublishedCatalog>` (root-neutral CRUD + multikey flip +
//! counter-only updates). Divergence tests (8-14) drive mutations
//! that MUST publish a fresh Arc (every DDL + root-moving CRUD).
//!
//! Identity is observed through the `__published_catalog_ptr` hidden
//! accessor: same pointer ⇒ same Arc (`Arc::ptr_eq` surrogate).
...
fn root_neutral_insert_reuses_catalog() {
    ...
    let pre = client.__published_catalog_ptr();
    col.insert_one(&doc! { "_id": 1i32 }).unwrap();
    let post = client.__published_catalog_ptr();
    assert_eq!(
        pre, post,
        "§10.8 #1: root-neutral insert must reuse catalog Arc"
    );
}
...
fn root_neutral_update_reuses_catalog() {
    ...
    assert_eq!(
        pre, post,
        "§10.8 #2: root-neutral update must reuse catalog Arc"
    );
}
...
fn root_neutral_delete_reuses_catalog() {
    ...
    assert_eq!(
        pre, post,
        "§10.8 #3: root-neutral delete (tombstone) must reuse catalog Arc"
    );
}
...
fn ready_secondary_insert_root_unchanged_reuses_catalog() {
    ...
    assert_eq!(
        pre, post,
        "§10.8 #4: Ready secondary insert with unchanged root must reuse catalog Arc"
    );
}
...
fn multikey_flip_reuses_catalog() {
    ...
    assert_eq!(
        pre, post,
        "§10.8 #5: multikey flip on Ready index must reuse catalog Arc (multikey is not published)"
    );
}
...
fn building_index_root_update_does_not_publish_during_iteration() {
    ...
    // ONLY a Building index root reuses the catalog Arc because
    ...
    // + commit publishes, and Arc identity changes ONLY across those
    ...
    col.create_index(IndexModel::builder().keys(doc! { "x": 1 }).build())
        .unwrap();
    let post = client.__published_catalog_ptr();
    // Reserve + Commit both publish new Arcs, so the final pointer
    // DIFFERS from the pre-create_index pointer. The "reuse" aspect is
...
fn entry_count_update_reuses_catalog() {
    ...
    assert_eq!(
        ...
        "§10.8 #7: entry_count / document_count bumps are not published \
         and must reuse catalog Arc"
    );
}

// ---------------------------------------------------------------------------
// §10.8 #8-14 — divergence tests (fresh Arc required)
// ---------------------------------------------------------------------------
...
fn create_namespace_publishes_new_catalog() {
    ...
    assert_ne!(
        pre, post,
        "§10.8 #8: create_namespace must publish a new catalog Arc"
    );
}

#[test]
fn create_index_reserve_publishes_new_catalog() {
    // The `reserve` step publishes a new Arc that makes the Building
    // entry visible to dual-writers. A reader that loads exactly once
    // after reserve sees the Building entry on the new Arc — witnessed
    // by Arc::ptr_eq inequality across the create_index call.
    ...
    assert_ne!(
        pre, post,
        "§10.8 #9: create_index_reserve must publish a new catalog Arc"
    );
}
...
fn create_index_commit_publishes_new_catalog() {
    ...
    // state Building→Ready and publishes a fresh Arc. To witness the
    // commit publish specifically, snapshot the pointer DURING the
    // build window is infeasible from integration tests, so we assert
    // that across the full create_index the Arc changes at least twice
    // (reserve + commit) by counting publishes elsewhere (US-012), and
    // at minimum the final-state Arc differs from the pre-state Arc.
    ...
    assert_ne!(
        pre, post,
        "§10.8 #10: create_index_commit must publish a new catalog Arc \
...
fn drop_index_publishes_new_catalog() {
    ...
    assert_ne!(
        pre, post,
        "§10.8 #11: drop_index must publish a new catalog Arc"
    );
}
...
fn drop_namespace_publishes_new_catalog_after_barrier() {
    ...
    assert_ne!(
        pre, post,
        "§10.8 #12: drop_namespace must publish a new catalog Arc \
...
fn ready_index_root_move_publishes_new_catalog() {
    ...
    // large payload dataset alongside long indexed keys. The Arc must
    // change at least once when the index root moves (§10.3 row:
    ...
    assert_ne!(
        pre, post,
        "§10.8 #13: a Ready index root split must publish a new catalog Arc \
...
fn data_root_move_publishes_new_catalog() {
    ...
    assert_ne!(
        pre, post,
        "§10.8 #14: data-tree root split must publish a new catalog Arc \
