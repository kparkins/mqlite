...
fn truncate_before_ready_transition(path: &Path) {
    let legacy_frames =
        crash_harness::scan_legacy_commit_frames(path).expect("scan legacy commit frames");
    let (ready_start, _ready_end) = legacy_frames
        .last()
        .copied()
        .expect("create_index must write a final Ready catalog commit");
    crash_harness::truncate_journal_to_offset(path, ready_start)
        .expect("truncate before Ready transition");
}

#[test]
fn test_class_b_b_index_build_in_progress_resumes_on_reopen() {
    ...
    collection
        ...
        .expect("create index");
    truncate_before_ready_transition(&path);
