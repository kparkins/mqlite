...
//! in document count), not quadratic.
//!
//! # Bug description
//!
//! Dropping the last `Client` handle triggers a checkpoint via
//! `src/client/handle.rs:148-158` → `ClientInner::checkpoint` →
//! `snapshot_ops::checkpoint` → `materialize_primary_deltas_for_checkpoint`.
//! That materialization path defers all B-tree leaf splits to checkpoint time,
//! so the work done during close is proportional to the *square* of the
//! documents inserted since open.
//!
...
#[test]
fn close_checkpoint_is_bounded_after_bulk_single_namespace_insert() {
    let dir = tempfile::tempdir().expect("create temp dir");
    let path = dir.path().join("close_checkpoint_bounded.mqlite");
    ...
    eprintln!(
        "[close_checkpoint_bounded] insert phase: {TOTAL_DOCS} docs in {insert_elapsed:.2?} \
         ({:.0} docs/s)",
    ...
    eprintln!(
        "[close_checkpoint_bounded] close (checkpoint) elapsed: {close_elapsed:.2?}  \
         (limit: {CLOSE_DEADLINE:.2?})"
    );

    assert!(
        close_elapsed < CLOSE_DEADLINE,
        "close-time checkpoint after {TOTAL_DOCS} docs took {close_elapsed:.2?}, \
         which exceeds the {CLOSE_DEADLINE:.2?} bound.\n\
         \n\
         This indicates the QUADRATIC checkpoint-materialization bug is present in \
         `materialize_primary_deltas_for_checkpoint`: the online CRUD path defers \
         all leaf splits to checkpoint time, making close cost O(n²) in document \
         count.\n\
         \n\
