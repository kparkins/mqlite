...
//!    and tested in `tests/mwmr_p5_crash.rs`).
...
/// US-030 — a first write to a missing namespace carries the allocated
/// durable `ns_id` out of bootstrap and admits that exact id before the
/// CRUD body runs. The test holds the writer at body entry, then proves
/// the registry lane for the published id is occupied. The source gate
/// rejects the stale pattern this story removes: bootstrap publish
/// followed by `id_for_name(ns)` recovery before admission.
#[test]
fn test_bootstrap_write_admits_allocated_ns_id_without_name_reresolve() {
    ...
    match client.__us036_close_and_drain(ns_id, 10) {
        Err(Error::WriterBusy) => {}
        other => panic!("allocated ns_id lane must be occupied by bootstrap writer, got {other:?}"),
    }
    ...
    assert!(
        engine_source.contains("admit(ns_id"),
        "bootstrap write path must admit the allocated ns_id directly",
    );
...
/// US-030 — if bootstrap publishes the namespace but writer admission
/// fails before the CRUD body, the namespace remains visible and the row
/// is not partially inserted.
#[test]
fn test_bootstrap_admission_busy_leaves_namespace_without_partial_insert() {
    let dir = tempfile::tempdir().unwrap();
    ...
    let err = coll
        .insert_one(&doc! { "_id": 31i32, "v": "no-partial" })
        .expect_err("closed allocated-id lane must reject bootstrap admission");
    ...
    assert_eq!(
        ...
        0,
        "admission failure must occur before the CRUD body inserts a row",
    );
}
...
fn test_unrelated_catalog_generation_advance_does_not_conflict() {
    ...
    // Publish a new catalog generation via DDL on a different
    // namespace. `create_collection` runs through `run_ddl`, which
    // takes `metadata.write()` (the writer dropped its read guard at
    // S1) and reserves the next `catalog_gen` under that lock.
    client
        .database(DB)
        .create_collection(COLL_B)
        .expect("DDL on a different namespace must proceed while CRUD is paused");
    ...
    // Release the writer. With the published generation now ahead of its
    // captured value, the S3.5 gate must revalidate COLL_A's identity and
    // allow the write because only COLL_B changed.
    hook.release()
        .expect("writer was waiting on release channel");

    let res = writer.join().expect("writer thread joined");
    res.expect("unrelated DDL must not conflict with target namespace write");
...
/// AC #6 / §10.8 CV-5 — a CRUD writer paused at body entry holds
/// neither `metadata.read()` nor `journal_mutex`; a concurrent DDL on a
/// DIFFERENT namespace must reach `metadata.write()` and publish
/// without blocking. The CRUD writer subsequently terminates (here:
/// with `CatalogGenerationChanged`, which is the in-memory rollback
/// path — never a deadlock or post-durable `WriteConflict`).
#[test]
fn test_ddl_during_writer_body_does_not_deadlock_or_post_durable_write_conflict() {
    ...
    // The DDL must not deadlock — give it a generous deadline and
    // assert it completes under that bound.
    let ddl_start = Instant::now();
    client
        .database(DB)
        .create_collection(COLL_B)
        .expect("DDL on a different namespace must proceed without deadlock");
    let ddl_elapsed = ddl_start.elapsed();
    assert!(
        ddl_elapsed < SETTLE_DEADLINE,
        "DDL took {ddl_elapsed:?}, exceeding the deadlock-detection bound {SETTLE_DEADLINE:?}",
    );

    // Releasing the writer must let it terminate. Whether it commits
    // or fails with `CatalogGenerationChanged` depends on whether the
    // DDL beat the writer's gate; either way it must NOT hang and
    // must NOT report a post-durable `WriteConflict`. The §10.21 CV-5
    // contract is that any `WriteConflict` returned by the publish
    // path is pre-durable.
    hook.release()
        .expect("writer was waiting on release channel");
    ...
    match res {
        ...
        other => panic!(
            "writer must succeed or return a pre-durable WriteConflict {{ CatalogGenerationChanged }}, got {other:?}",
        ),
    }
}
...
/// US-024 — while `drop_namespace` is draining a held writer, the namespace
/// writer-registry lane is closed and direct admissions fail with
/// `WriterBusy`. Guard drop after successful publish removes the old lane, so
/// a later recreate starts with a clean writer lane.
#[test]
fn test_drop_namespace_admit_refused_during_drain() {
    ...
    let deadline = Instant::now() + SETTLE_DEADLINE;
    loop {
        match client.__us036_admit_writer(ns_id, 0) {
            Err(Error::WriterBusy) => break,
            Ok(ticket) => drop(ticket),
            Err(err) => panic!("expected direct admit to reach WriterBusy, got {err:?}"),
        }
        assert!(
            Instant::now() < deadline,
            "drop_namespace did not close the writer lane within {SETTLE_DEADLINE:?}",
        );
        assert!(
            !drop_done.load(Ordering::Acquire),
            "drop_namespace completed before the held writer was released",
        );
        thread::sleep(Duration::from_millis(5));
    }
...
fn test_create_index_cleanup_revalidates_ns_id_under_metadata_write() {
    ...
    let writer = thread::spawn(move || -> Result<(), Error> {
        writer_client
            .database(DB)
            .collection::<Document>(COLL_A)
            .insert_one(&doc! { "_id": 3803i32, "tag": "recreated-held" })
            .map(|_| ())
    });
    writer_hook
        .wait_until_entered_timeout(SETTLE_DEADLINE)
    ...
    build_hook
        .release()
        .expect("build was waiting on release channel");
    let cleanup_finished_while_writer_held = flag_set_within(&build_done, SETTLE_DEADLINE);
    ...
    assert!(
        cleanup_finished_while_writer_held,
        "cleanup must reject the stale namespace identity without draining the recreated namespace lane",
    );
    assert_eq!(
        client.__published_catalog_gen(),
        gen_after_recreate,
