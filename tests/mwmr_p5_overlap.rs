...
#[test]
#[serial(mwmr_p5_overlap_hooks)]
fn test_crud_on_existing_leaf_does_not_write_overlay_bytes() {
    let dir = tempfile::tempdir().unwrap();
    ...
    client.__us008_reset_overlay_observations();
    coll.insert_one(&doc! { "_id": 2i32, "name": "root-neutral" })
        .unwrap();

    assert_eq!(
        client.__us008_committed_overlay_leaf_bytes(),
        0,
        "root-neutral CRUD on an existing leaf must not commit logical row bytes through TxnOverlay",
    );
    assert_eq!(
